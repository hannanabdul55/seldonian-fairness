...
def ghat_tpr_diff(A_idx, method='ttest', threshold=0.2):
    def tpr_ab(X, y_true, y_pred, delta, n=None):
        ...
        tp = tpr_rate()(X, y_true, y_pred)

        if method == 'ttest':
            bound = abs(ttest_bounds(tp_b, delta, n) - ttest_bounds(tp_a, delta, n))
        else:
            bound = abs(hoeffdings_bounds(tp_b, delta, n) - hoeffdings_bounds(tp_a, delta,
                                                                            n))
