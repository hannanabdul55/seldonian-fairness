...
def plot_results_linreg(res, opt, display=True, axs=None, show_subtitle=False):
    if axs is None:
        fig, axs = plt.subplots(4, 1, figsize=(6.4, 4.8*4))
    x = []
    sol_found_c = []
    ...
    if display:
        ...
        ax = axs[3]
        ax.plot(x, acc_c, label='Constrained mse')
        ax.plot(x, acc_uc, label='Unconstrained mse')
        ax.set_xlabel('Number of data points')
        ax.set_ylabel('Accuracy')
        ax.set_xscale('log')
        ax.legend()
        ax.grid(True)
        if show_subtitle:
            ax.set_title(f"[{opt}]data vs MSE")
        else:
            ax.set_title(f"[{opt}]")
        # ax.show()
    return x, sol_found_c, sol_found_c_std, ghat_c, ghat_uc, fr_c, fr_c_std, fr_uc, fr_uc_std, acc_c, acc_uc
