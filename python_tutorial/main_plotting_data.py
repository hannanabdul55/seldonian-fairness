...
res_path = '/mnt/nfs/work1/pthomas/akanji/res_dist_control'
...
def run_experiments(worker_id, nWorkers, ms, numM, numTrials, mTest):
	...
	for trial in range(numTrials):
		for (mIndex, m) in enumerate(ms):
			...
			print(f"[(worker {worker_id}/{nWorkers}) LeastSq   trial {trial+1}/{numTrials}, m {m}] distance: {dists[trial, mIndex]}")
		print()
