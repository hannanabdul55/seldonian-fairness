        ...
        while counteval < self.stopeval:
            ...
            for k in range(lambda_p):
                ...
                arfitness[k] = self.loss(X=X, y_true=y,
                                         theta=arx[:, k])
                counteval += 1
            B, C, D, arfitness, arindex, sigma, xmean, eigenval, invsqrtC = run_cma_iter(
                B, C, N, arfitness, arx,
                c1, cc, chiN, cmu,
                counteval, cs, damps,
                eigenval, invsqrtC,
                lambda_p, mu, mueff, pc,
                ps, sigma, weights,
                xmean, self.verbose
            )
...
@njit
def run_cma_iter(B, C, N, arfitness, arx, c1, cc, chiN, cmu, counteval, cs, damps,
                 eigenval, invsqrtC, lambda_p, mu, mueff, pc, ps, sigma, weights, xmean,
                 verbose
                 ):
    ...
    if verbose:
        print(
            "Current evaluation:",counteval, "\t average loss:",arfitness[1])
    xold = np.copy(xmean)
    ...
    return B, C, D, arfitness, arindex, sigma, xmean, eigenval, invsqrtC
